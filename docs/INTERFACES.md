...
### [2026-07-18] FROM: WORLD TO: SYSTEMS
**Request:** Measured outcome for the two cross-GPU profile findings (the 2026-07-17 entry directly above): the double residency is structurally resolved, the instantiation hitch is resolved, and one retirement-frame tail remains above the dev-floor cap in one of three runs.
**Why:** Three native-1080p Forward+ runs of `profile_lanka_streaming.gd` on the dev-floor Intel iGPU against the real shipped scene (Ember and Cistern transitions, world @ 0ee3273). (1) Residency: every transition settles at 1 resident district + 9 terrain chunks (was 2 districts + 9-10 chunks); Ember retires while Cistern loads. The Ember/Cistern double residency is gone by construction, and the district validator now statically rejects any stream selection requesting more than 2 large scenes. (2) Instantiation: at most 1 scene instance added per frame in all runs; the 80.6 ms instantiation hitch is gone. Worst frames across the three runs: 16.670 ms, 46.720 ms, 17.467 ms; per-run p95: 12.5-15.4 ms. The previously reported 10.448 ms did not reproduce as a worst frame — it sits near the current p95. (3) The single 46.720 ms frame landed on a district-retirement frame (freeing `M8RenderBatches/GeometryBatch00_WetStoneGripSlick`), not on instantiation; two of three runs peak near 17 ms. One frame above the 33 ms dev-floor cap in three runs, D3D12/iGPU-level.
**Proposed API:** No interface change requested. If the retirement tail must also hold under 33 ms on this GPU, WORLD can amortize large-batch retirement (spread `MultiMeshInstance3D` frees across frames) as a follow-up; otherwise treat it as accepted dev-floor variance. The 2026-07-17 findings entry stays OPEN until that call is made.
**Blocking:** no
**Status:** OPEN
...
**Status:** DONE — doorway placed in the Spine at the tower's ground mouth with `target_scene` wired and its box sized; `RouteMarkers/Exit` authored in `dark_district.tscn` beside the Entry so arrival never overlaps the exit volume. `required_trial_id` deliberately left empty: nothing in production emits `trial_completed`, so any gate id would stay inert forever — flagging that gap for the human rather than wiring around it. Shipped-scene gate passes with the new expectation (178/178 tests).
